document.addEventListener("DOMContentLoaded", function () {  //start the script after loading page
  function updateProfileInfo(profileData) {                  //function to change profile info
    // photo -----------------------
  ...
  function updateSoftSkills(profileData){                    //function to change softSkills
    const softSkills = document.querySelector(".profile.skills.softSkills")
    softSkills.innerHTML = profileData.skills.softSkills.map(skill => `<li>${skill}</li>`).join('')
    return(profileData)  // to the next promisse this line is necessary
  }
  function updateHardSkills(profileData){                    //function to change hardSkills    
    const hardSkills = document.querySelector(".profile.skills.hardSkills")
    hardSkills.innerHTML = profileData.skills.hardSkills.map(skill => `<li><img src="${skill.logo}" alt="${skill.name}" title="${skill.name}"></li>`).join('')
    return(profileData)  // to the next promisse this line is necessary
  }
  function updatelanguages(profileData){                    //function to change languages skills
    const languages = document.querySelector(".profile.languages")
    languages.innerHTML = profileData.languages.map(language => `<li>${language}</li>`).join('')
    return(profileData)  // to the next promisse this line is necessary
  }
  function updatePortfolio(profileData){                    //function to change portfolio skills
    ...
    function updateExperience(profileData){                    //function to change experience skills
  ...
  function loadProfile() {                                //funtion to fetch to profile from the json to this script
...
// call to the function and sequece of promisses to update the profile info, softSkills, hardSkills, languages and portfolio.
  loadProfile()
